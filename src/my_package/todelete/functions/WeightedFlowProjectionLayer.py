...
class WeightedFlowProjectionLayer(Function):
    ...
    @staticmethod
    def forward(ctx, input1,input2,input3, threshold, requires_grad):
        ...
        if input1.is_cuda :
            ...
            err = my_lib.WeightedFlowProjectionLayer_gpu_forward(input1, input2,input3,
                                                                 count,weight,  output,  fillhole, threshold)
        else:
        ...
        ctx.save_for_backward(input1, input2, input3, count, weight)
    ...
    def backward(ctx,  gradoutput):
        ...
        input1, input2, input3, count, weight = ctx.saved_tensors
        ...
        if input1.is_cuda:
            ...
            err = my_lib.WeightedFlowProjectionLayer_gpu_backward(input1, input2, input3,
                                                                  count, weight, gradoutput, gradinput1,  threshold)
            # print(err)
            if err != 0 :
                print(err)
        else:
            ...
            gradinput3 = torch.FloatTensor().resize_(input3.size()).zero_()
            err = my_lib.WeightedFlowProjectionLayer_cpu_backward(input1, input2,  input3,
                                                                  count, weight,  gradoutput, gradinput1, threshold)
            # print(err)
            if err != 0:
                print(err)
        ...
        # print(gradinput1)
        return gradinput1,gradinput2 ,gradinput3, None,None
