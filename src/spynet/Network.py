...
# 	if strOption == '--model':
# arguments_strModel = 'sintel-final' # which model to use, see below
#
	...
	def __init__(self):
		...
		class Preprocess(torch.nn.Module):
			...
			def forward(self, variableInput):
				variableBlue = variableInput[:, 0:1, :, :].clone() - 0.406
				variableGreen = variableInput[:, 1:2, :, :].clone() - 0.456
				variableRed = variableInput[:, 2:3, :, :].clone() - 0.485
			...
			def __init__(self, intLevel):
				...
				for intConv in range(5):
					...
					self.moduleBasic[intConv * 2].weight.data.copy_(torch.utils.serialization.load_lua(
                        os.path.dirname(os.path.abspath(__file__)) +'/models/modelL' + str(intLevel + 1) + '_' + arguments_strModel  + '-' + str(intConv + 1) + '-weight.t7'))
					self.moduleBasic[intConv * 2].bias.data.copy_(torch.utils.serialization.load_lua(
                        os.path.dirname(os.path.abspath(__file__)) + '/models/modelL' + str(intLevel + 1) + '_' + arguments_strModel  + '-' + str(intConv + 1) + '-bias.t7'))
				# end
			# end
		...
		class Backward(torch.nn.Module):
			...
			# 	return torch.nn.functional.grid_sample(input=variableInput, grid=(variableGrid + variableFlow).permute(0, 2, 3, 1), mode='bilinear', padding_mode='border')
				return torch.nn.functional.grid_sample(input=variableInput, grid=(variableGrid + variableFlow).permute(0, 2, 3, 1), mode='bilinear')
		# end
...
#
#
# 	offset = numpy.transpose(tensorOutput.numpy(),(1, 2, 0))
# 	flow = flowToColor(offset)
# 	imsave(arguments_strOut[:-4] + '_offset.png', flow)
#
