...
__all__ = ['S2DF','S2DF_3dense','S2DF_3dense_reviseCtx','S2DF_3dense_nodilation',
...
class BasicBlock_reviseCtx(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, dilation = 1, stride=1, downsample=None):
        super(BasicBlock_reviseCtx, self).__init__()
        self.conv1 = conv3x3(inplanes, planes,dilation, stride)
        # self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
...
class S2DF_reviseCtx(nn.Module):

    def __init__(self, block, num_blocks,dense = True,dilation=True):
        self.inplanes = 64
        super(S2DF_reviseCtx, self).__init__()
        self.dense = dense
        self.num_block = num_blocks
        assert(num_blocks>=1 and num_blocks<=4)
        self.block1 = nn.Sequential(*[
    ...
    def forward(self, x):
        ...
        x = self.block2(self.relu(x)) if self.num_block>=2 else x
        if (self.num_block > 2 and self.dense) or self.num_block == 2:
            y.append(x)

        x = self.block3(self.relu(x)) if self.num_block>=3 else x
        if (self.num_block > 3 and self.dense) or self.num_block == 3:
            y.append(x)

        x = self.block4(self.relu(x)) if self.num_block== 4 else x
        if self.num_block == 4 :
            y.append(x)
...
def S2DF_3dense_reviseCtx():
    model = S2DF_reviseCtx(BasicBlock_reviseCtx,3,dense=True)
    return model
