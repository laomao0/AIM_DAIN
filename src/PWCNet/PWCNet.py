...
class PWCDCNet(nn.Module):
    ...
    def forward(self,x, output_more = False):
        im1 = x[:,:3,:,:] # S x 3 x H x W
        im2 = x[:,3:,:,:]
        # print("\n\n***************************PWC Net details *************** \n\n")
        # start=  time.time()
        c11 = self.conv1b(self.conv1aa(self.conv1a(im1))) # 1 x 16 x H/2 W/2
        c21 = self.conv1b(self.conv1aa(self.conv1a(im2)))
        c12 = self.conv2b(self.conv2aa(self.conv2a(c11))) # 1 x 32 x H/4 W/4
        c22 = self.conv2b(self.conv2aa(self.conv2a(c21)))
        c13 = self.conv3b(self.conv3aa(self.conv3a(c12))) # 1 x 64 x H/8 W/8
        c23 = self.conv3b(self.conv3aa(self.conv3a(c22)))
        c14 = self.conv4b(self.conv4aa(self.conv4a(c13))) # 1 x 96 x H/16 W/16
        c24 = self.conv4b(self.conv4aa(self.conv4a(c23)))
        c15 = self.conv5b(self.conv5aa(self.conv5a(c14))) # 1 x 128 x H/32 W/32
        c25 = self.conv5b(self.conv5aa(self.conv5a(c24)))
        c16 = self.conv6b(self.conv6a(self.conv6aa(c15))) # 1 x 196 x H/64 W/64
        c26 = self.conv6b(self.conv6a(self.conv6aa(c25)))
        # print("features " +str(time.time()- start))
        # start=  time.time()
        corr6 = self.corr(c16, c26) 
        corr6 = self.leakyRELU(corr6)   # 1 x 81 x H/64 W/64


        x = torch.cat((self.conv6_0(corr6), corr6),1) # 1 x 209 x H/64 W/64
        x = torch.cat((self.conv6_1(x), x),1) # 1 x 377 x H/64 W/64
        x = torch.cat((self.conv6_2(x), x),1)  # 1 x 433 x H/64 W/64
        x = torch.cat((self.conv6_3(x), x),1) # 1 x 497 x H/64 W/64
        x = torch.cat((self.conv6_4(x), x),1) # 1 x 529 H/64 W/64
        flow6 = self.predict_flow6(x) # 1 x 2 x H/64 W/64
        up_flow6 = self.deconv6(flow6) # 1 x 2 x H/32 W/32
        up_feat6 = self.upfeat6(x)  # 1 x 2 x H/32 W/32
        # print("level6 " +str(time.time()- start))
        # start=  time.time()
        
        warp5 = self.warp(c25, up_flow6*0.625)  # 1 x 128 x H/32 W/32
        ...
        flow5 = self.predict_flow5(x)  # 1 x 2 x H/32 W/32
        up_flow5 = self.deconv5(flow5)
        up_feat5 = self.upfeat5(x)  # 1 x 2 x H/16 W/16
        # print("level5_3 " + str(time.time() - start5))
        # print("level5 " + str(time.time() - start))
        # start = time.time()

        warp4 = self.warp(c24, up_flow5*1.25)   # 1 x 96 H/16 W/16
        ...
        flow4 = self.predict_flow4(x)  # 1 x 2 H/16 W/16
        ...
        warp3 = self.warp(c23, up_flow4*2.5) # 1 x 64 x H/8 W/8
        ...
        flow3 = self.predict_flow3(x)  # 1 x 2 H/8 W/8
        ...
        warp2 = self.warp(c22, up_flow3*5.0) # 1 x 32 x H/4 W/4
        ...
        flow2 = self.predict_flow2(x)  # H/4 W/4
